...
namespace WebUygulamaProje1.Models
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public T Get(Expression<Func<T, bool>> filtre, string? inculedeProps = null)
        {
            IQueryable<T> sorgu = dbSet;
            sorgu = sorgu.Where(filtre);

			if (!string.IsNullOrEmpty(inculedeProps))
			{
				foreach (var includeProp in inculedeProps.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
				{
        ...
        public IEnumerable<T> GetAll(string? inculedeProps = null)
        {
            IQueryable<T> sorgu = dbSet;
